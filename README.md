...
3. Run `nextflow [path to tardis-phylogenetics]/tardis example`
...
50 jobs (1 per batch) for each generation. When all the jobs will be completed, the 50 batch for the new generation will be submitted, and so on.
...
TARDiS has been successfully used on Linux Ubuntu (local) and SLURM (hpc). Please let use know if you are using on other platforms.
